...
const WantToChatCircle = ({setIsOpen}: {setIsOpen: (isOpen: boolean) => void, isOpen: boolean}) => {
  return (
    <section className=" w-[350px] rounded-full p-4">
        <div className="flex justify-end mb-2 mr-5">
        <MdCancel onClick={() => setIsOpen(true)} size={22} className="text-gray-600 cursor-pointer" />
        </div>
